/**
 * Crear clientes en Alegra a partir de los datos de BigQuery (WooCommerce US).
 */

import { getAlegraSecrets } from '../../config/secrets';
import type { WoocommerceUsClientRow } from './query';
...
function buildPayload(row: WoocommerceUsClientRow): AlegraPayload {
...
export async function createClient(
  row: WoocommerceUsClientRow
): Promise<{ ok: boolean; status: number; body: string }> {
  const { token, email } = await getAlegraSecrets();
  const authHeader = 'Basic ' + Buffer.from(`${email}:${token}`).toString('base64');
  const payload = buildPayload(row);

  console.log('[Alegra Client Woocommerce US] Datos a cargar:', JSON.stringify(payload));
  ...
  console.log(
    '[Alegra Client Woocommerce US] Respuesta Alegra:',
    JSON.stringify({ status: res.status, ok: res.ok, body })
  );
