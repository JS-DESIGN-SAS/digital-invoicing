/**
 * Crear clientes en Alegra a partir de los datos de BigQuery (Meli / Mercado Libre).
 */

import { getAlegraSecrets } from '../../config/secrets';
import type { MeliClientRow } from './query';
...
function buildPayload(row: MeliClientRow): AlegraPayload {
  ...
  const payload: AlegraPayload = {
    ...
    mobile: row.phone ?? '',
    email: row.email || 'contacto@julianasanchez.co',
    type: 'client',
  };
...
export async function createClient(row: MeliClientRow): Promise<{ ok: boolean; status: number; body: string }> {
  const { token, email } = await getAlegraSecrets();
  const authHeader = 'Basic ' + Buffer.from(`${email}:${token}`).toString('base64');
  const payload = buildPayload(row);

  console.log('[Alegra Client Meli] Datos a cargar:', JSON.stringify(payload));
  ...
  console.log('[Alegra Client Meli] Respuesta Alegra:', JSON.stringify({ status: res.status, ok: res.ok, body }));
